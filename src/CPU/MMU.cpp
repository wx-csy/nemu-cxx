...
template <>
uint32_t vaddr_read<uint32_t>(vaddr_t addr) {
...
template <>
uint16_t vaddr_read<uint16_t>(vaddr_t addr) {
...
template <>
uint8_t vaddr_read<uint8_t>(vaddr_t addr) {
...
template <>
void vaddr_write(vaddr_t addr, uint32_t data) {
  if ((addr & 3) == 0) memory[address_translate(addr)] = data;
  else {
    (memory[address_tranlsate(addr & -4)] 
      &= (0xffffffff >> ((addr & 3) * 8)))
      |= data << ((addr & 3) * 8);
...
template <>
void vaddr_write(vaddr_t addr, uint16_t data) {
  if ((addr & 3) != 3) {
...
template <>
void vaddr_write(vaddr_t addr, uint8_t data) {
  (memory[address_translate(addr & -4)] 
    &= ~(0xff << ((addr & 3) * 8)) 
    |= data << ((addr & 3) * 8);
}
